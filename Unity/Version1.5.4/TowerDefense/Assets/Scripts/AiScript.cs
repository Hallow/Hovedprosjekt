...
public class AiScript : MonoBehaviour {
	...
	void Update () {
        ...
        {
            turnController.GetComponent<TurnScript>().enemyReady = false;
        }

        if (!turnController.GetComponent<TurnScript>().enemyReady && turnController.GetComponent<TurnScript>().playerReady && turnController.GetComponent<TurnScript>().turnNumber == 0)
        {
            round1();
        }

        if(!turnController.GetComponent<TurnScript>().enemyReady && turnController.GetComponent<TurnScript>().playerReady && turnController.GetComponent<TurnScript>().turnNumber == 1)
        {
            round2();
        }

        if (!turnController.GetComponent<TurnScript>().enemyReady && turnController.GetComponent<TurnScript>().playerReady && turnController.GetComponent<TurnScript>().turnNumber == 2)
        {
            round3();
        }
